...
typedef SequencerPortList<int> testlist_t;
testlist_t testlist;
...
void check_testlist(const std::string & s) {
  ...
  for(testlist_t::LockedItemIterator i(testlist);
      i != testlist.end(); ++i) {
    out << *(i->port) << " " << std::flush;
  }
