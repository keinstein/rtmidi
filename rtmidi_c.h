...
extern "C" {
  namespace rtmidi {
    struct RtMidiCompatibility;
  }
  typedef rtmidi::RtMidiCompatibility RtMidi;
#else
  struct RtMidiCompatibiltiy;

  typedef struct RtMidiCompatibiltiy RtMidi;
#endif
...
//! Compatibility typedefs for RtMidiInPtr and RtMidiOutPtr
typedef RtMidiPtr RtMidiInPtr;
typedef RtMidiPtr RtMidiOutPtr;
...
//! Create a default RtMidiPtr value, with no initialization.
RTMIDIAPI RtMidiPtr rtmidi_in_create_default (void);

/*! Create a  RtMidiPtr value, with given api, clientName and queueSizeLimit.
 *
 ...
 *                        specified.
 */
RTMIDIAPI RtMidiPtr rtmidi_in_create (enum RtMidiApi api, const char *clientName, unsigned int queueSizeLimit);

//! Deallocate the given pointer.
RTMIDIAPI void rtmidi_in_free (RtMidiPtr device);
...
RTMIDIAPI void rtmidi_in_set_callback (RtMidiPtr device, RtMidiCCallback callback, void *userData);

//! Cancel use of the current callback function (if one exists).
RTMIDIAPI void rtmidi_in_cancel_callback (RtMidiPtr device);

//! Specify whether certain MIDI message types should be queued or ignored during input.
RTMIDIAPI void rtmidi_in_ignore_types (RtMidiPtr device, bool midiSysex, bool midiTime, bool midiSense);
 ...
 */
RTMIDIAPI double rtmidi_in_get_message (RtMidiPtr device, unsigned char *message, size_t *size);

/* RtMidiOut API */

//! Create a default RtMidiPtr value, with no initialization.
RTMIDIAPI RtMidiPtr rtmidi_out_create_default (void);

/*! Create a RtMidiPtr value, with given and clientName.
 *
 *  \param api            An optional API id can be specified.
 *  \param clientName     An optional client name can be specified. This
 *                        will be used to group the ports that are created
 *                        by the application.
 */
RTMIDIAPI RtMidiPtr rtmidi_out_create (enum RtMidiApi api, const char *clientName);

//! Deallocate the given pointer.
RTMIDIAPI void rtmidi_out_free (RtMidiPtr device);
...
RTMIDIAPI int rtmidi_out_send_message (RtMidiPtr device, const unsigned char *message, int length);
